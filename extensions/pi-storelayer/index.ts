...
const DOMAIN_GUIDELINES: Record<string, string[]> = {
  ...
  storage: [
    "Use 'storage.create_table' to create typed tables (column types: text, integer, real, boolean, json, datetime).",
    "Use 'storage.execute_sql' to run raw SQL queries — supports SELECT, INSERT, UPDATE, DELETE, CREATE TABLE, etc.",
    "Use 'storage.query_rows' for structured queries with filters (eq, gt, contains, in, etc.), sort, and pagination.",
    "Every row must have an 'id' field. Auto-generated columns: id, created_at, updated_at.",
    "Tables can be used as 'custom_table' resources in rules — set lookupField + lookupExpression for row-level lookups.",
    "Limits: 20 tables, 50 columns/table, 50K rows/table, 10 indexes/table.",
  ],
  events: [
